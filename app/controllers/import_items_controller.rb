class ImportItemsController < ApplicationController
  ...
  def history
    if !params[:searchValue].blank? || !params[:daterange].blank?
      ...
      @import_items = @import_items.where("imports.bl_number ILIKE :query OR import_items.container_number ILIKE :query OR imports.work_order_number ILIKE :query OR trucks.reg_number ILIKE :query OR import_items.truck_number ILIKE :query", query: "%#{params[:searchValue]}%") unless params[:searchValue].blank?
