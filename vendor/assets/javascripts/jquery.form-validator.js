    ...
    */
    $.formUtils.addValidator({
        ...
        validatorFunction : function(email) {
            var result=true
            var emails = email.toLowerCase().split(',');
            if (emails.length>1){
              for(var i=0;i<emails.length;i++) {
                 
                  var emailParts=emails[i].toLowerCase().split('@');
                  if( emailParts.length == 2 ) {
                  result=result && $.formUtils.validators.validate_domain.validatorFunction(emailParts[1]) &&
                        !(/[^\w\+\.\-]/.test(emailParts[0]));
                  }
                  else{
                    result=result&&false;
