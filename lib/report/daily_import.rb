  ...
  class DailyImport
    ...
    def create(customer)
      ...
      workbook.styles do |s|
        ...
        { 'In Transit' => nil}.each do |name, status|
          workbook.add_worksheet(name: name) do |sheet|
            add_data(imports, sheet, center, heading, status)
          end
        end
      end
    ...
    def add_data(imports, sheet, center, heading, status)
          ...
          [import,item].each do |entity|
            entity.audits.collect(&:audited_changes).each do |a|
              if !a[:status].blank? then
                array.concat(a[:updated_at].second.to_date.strftime("%d-%b-%Y") + " " +
                             a[:status].second + "\n")
            ...
            entity.remarks.nil? ? array : array.concat("Remarks:" + entity.remarks + "\n")
          end

          sheet.add_row [import.work_order_number, import.bl_number,
                     item.container_number, import.equipment, import.description,
                     import.estimate_arrival.to_date.strftime("%d-%b-%Y"), out_of_port_date,
                     array, item.truck_number, item.bond_direction, item.bond_number],
                     style: center, height: 180
